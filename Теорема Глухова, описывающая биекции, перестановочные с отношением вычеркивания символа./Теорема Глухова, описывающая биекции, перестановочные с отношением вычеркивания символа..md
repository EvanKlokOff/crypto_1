...
Если $e(x) \rho ^{|x|-1}e(\lambda)$ , то т.к  $e^{-1}\in Z(\rho)$, то $x \rho ^{|x|-1}\lambda \bigotimes$

![alt text](image-1.png) 
* **Тут должно быть равенство вместо $\le$**
...
\end{cases}$
## Д-ВО $\Leftarrow$
д-но, .к проверили, что $\Pi, \theta, \Pi \circ \theta \in Z(\rho)$
...
* в силу лем 3 и 4, т.е отображение сохраняет длину и содержание
...
т.е можно выбрав $\varphi = \begin{cases}
    либо \ \Pi^{-1} \circ e \\
...
$\Pi^{-1} \circ e (abc) = w, где S(w)= \{a,b,c\}, |w|=3$ 
...
приходим к противоречию $\bigotimes$
...
* $\varphi_{\{a_1,a_2,a_3\}}$ и $\varphi_{\{a_1,a_2,a_4\}}$ действуют тождественно на a_1a_2

т.е $\varphi_{\{a_1,a_2,a_3\}}$ =  $\varphi_{\{a_1,a_2,a_4\}}$

Заметим, что $\{a_1, a_2, a_3, a_4\}^2 \subseteq \{a_1,a_2,a_3\}^2 \cup \{a_1, a_2, a_4\}$
...
$\varphi(a_1, a_2) = a_1a_2 = \varphi_{\{a_1\cdots a_k\}}(a_1, a_k)\Rightarrow \varphi = \varphi_{\{a_1, \cdots, a_k\}}$
...
* по предложению $k+1 \ge 3, \varphi(w) = w$
